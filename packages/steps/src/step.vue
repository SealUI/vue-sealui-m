<template>
    ...
    <div
      class="sealui-step__head"
      :class="['is-' + currentStatus, { 'is-text': !icon }]">
      <div
        class="sealui-step__line"
        :style="isLast ? '' : { marginRight: $parent.stepOffset + 'px' }"
        :class="['is-' + $parent.direction, { 'is-icon': icon }]">
        <i class="sealui-step__line-inner" :style="lineStyle"></i>
      </div>

      <span class="sealui-step__icon">
        <slot
          v-if="currentStatus !== 'success' && currentStatus !== 'error'"
          name="icon">
          <i v-if="icon" :class="['sealui-icon-' + icon]"></i>
          <div v-else>{{ index + 1 }}</div>
        </slot>
        <i
          v-else
          :class="['sealui-icon-' + (currentStatus === 'success' ? 'checkmark-round' : 'close')]">
        </i>
      </span>
    </div>
    <div
      class="sealui-step__main"
      :style="{ marginLeft: mainOffset }">
      <div
        class="sealui-step__title"
        ref="title"
        :class="['is-' + currentStatus]">
        <slot name="title">{{ title }}</slot>
      </div>
      <div
        class="sealui-step__description"
        :class="['is-' + currentStatus]">
