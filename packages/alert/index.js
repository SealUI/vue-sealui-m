 ...
 * @example
 * this.$alert('普通的alert');
 * this.$alert('带回调的alert', function(){ console.log('ok') });
 * var alertDom = this.$alert('手动关闭的alert', function(){
 *     return false; // 不关闭弹窗，可用alertDom.hide()来手动关闭
 * });
 * this.$alert('自定义标题的alert', { title: '自定义标题' });
 * this.$alert('带回调的自定义标题的alert', function(){
 *    console.log('ok')
 * }, {
 *    title: '自定义标题'
 * });
 * this.$alert('自定义按钮的alert', {
 *     title: '自定义按钮的alert',
 *     buttons: [{
 *         label: 'OK',
 *         type: 'primary',
